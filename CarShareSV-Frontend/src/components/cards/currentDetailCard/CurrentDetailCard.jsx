...
export default function CurrentDetailCard({ rent }) {
  const { cancelReservation, getUserReservations } = useReservation();
...
const handleOnClick = async () => {
  ...
  try {
    ...
    // Refresca las reservas del usuario para actualizar la UI
    if (user?.userId) {
      await getUserReservations(user.userId);
    }
  ...
  // Deshabilita el botón si la reserva no es cancelable
